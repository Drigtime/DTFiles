...
Object.keys(elementLooks).forEach(prop => {
  if (!fs.existsSync(`./resources/${prop}`)) fs.mkdirSync(`./resources/${prop}`);
  Object.keys(elementLooks[prop]).forEach(element => {
    ...
    Object.values(D2O).forEach(map => {
      let mapJson;
      if (fs.existsSync(`./D2P/${map.id}.json`)) mapJson = JSON.parse(fs.readFileSync(`./D2P/${map.id}.json`));
      else return;
      let quant = 0;
        ...
        if (Array.isArray(cell)) {
          cell.forEach(elementOnCell => {
            if (elementOnCell.look) {
              if (elementOnCell.look == elementLooks[prop][element] && elementOnCell.x > 0 && elementOnCell.y > 0) {
                quant += 1;
              }
            }
          });
